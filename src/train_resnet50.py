...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a pretrained ResNet50 on Kaggle chest X-ray split CSV.")
    parser.add_argument("--csv_path", required=True, help="Path to split CSV.")
    parser.add_argument("--out_dir", default="outputs/resnet50", help="Output directory.")
...
def main():
    ...
    model = ResNet50Binary(pretrained=True).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr)

    best_auc = -math.inf
    best_checkpoint_path = out_dir / f"best_resnet50_seed{args.seed}.pt"
